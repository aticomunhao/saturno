...
@section('content')
    <br>
    <div class="container">
        <form action="{{ route('conta_contabil.update', $contaContabil->id) }}" method="POST">
            @csrf
            @method('PUT')
                            ...
                            <div class="col-md-2">
                                ...
                                <select class="form-select" name="id_tipo_catalogo" id="idcatalogocontabil" required>
                                    @foreach ($catalogo_conta_contabil as $conta_contabil)
                                        <option value="{{ $conta_contabil->id }}" {{ $conta_contabil->id == $contaContabil->id_tipo_catalogo ? 'selected' : '' }}>{{ $conta_contabil->nome }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4">
                                <label for="iddescricao" class="form-label">Descrição:</label>
                                <input type="text" name="descricao" id="iddescricao" class="form-control" placeholder="Descrição Conta Contábil" value="{{ $contaContabil->descricao }}" required>
                            </div>
                            <div class="col-md-2">
                                <label for="idnaturezacontabil" class="form-label">Natureza:</label>
                                <select name="id_tipo_natureza_conta_contabil" id="idnaturezacontabil" class="form-select" required>
                                    @foreach ($natureza_conta_contabil as $natureza_contabil)
                                        <option value="{{ $natureza_contabil->id }}" {{ $natureza_contabil->id == $contaContabil->id_tipo_natureza_conta_contabil ? 'selected' : '' }}>{{ $natureza_contabil->nome }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-2">
                                <label for="idgrupocontabil" class="form-label">Grupo:</label>
                                <select name="id_tipo_grupo_conta_contabil" id="idgrupocontabil" class="form-select" required>
                                    @foreach ($grupo_conta_contabil as $grupo_contabil)
                                        <option value="{{ $grupo_contabil->id }}" {{ $grupo_contabil->id == $contaContabil->id_tipo_grupo_conta_contabil ? 'selected' : '' }}>{{ $grupo_contabil->nome }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-2">
                                <label for="idgrau" class="form-label">Grau:</label>
                                <input type="text" name="grau" id="idgrau" class="form-control" value="{{ $contaContabil->grau }}" readonly>
                            </div>
                        </div>
                    </fieldset>
                    <h4>Níveis</h4>
